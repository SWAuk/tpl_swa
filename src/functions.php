...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the swa-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"swa-post" . $classes . "\">");
            artxFragmentBegin("<div class=\"swa-postmetadataheader\">");
            artxFragmentBegin("<h2 class=\"swa-postheader\">");
            ...
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"swa-postheadericons swa-metadata-icons\">");
            if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
                foreach ($data['metadata-header-icons'] as $icon)
                    artxFragment('', $icon, '', ' | ');
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"swa-postcontent swa-postcontent-0 clearfix\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"swa-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"swa-blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"swa-blockcontent\">");
     ...
     *  Select inputs with class != 'swa-button'
     *   artxFindTags($html, array('input' => array('class' => '!swa-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'swa-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!swa-button', '!'))))
     *  Select inputs with class = button but != 'swa-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!swa-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'swa-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'swa-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    function artxPostprocessContent($content)
    {
        ...
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!swa-button')),
            'button' => array('class' => array('button', '!swa-button')), 'button' => array('class' => array('btn', '!swa-button'))));
        return $content;
    }
